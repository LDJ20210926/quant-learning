...
class stock_pool_strategies():
    
    def __init__(self,begin_date=None,end_date=None,adjust_interval=None,pe_range=None):
        self.daily=DB_CONN["daily"]
        self.daily_hfq=DB_CONN["daily_hfq"]
        self.begin_date=begin_date
        self.end_date=end_date
        self.adjust_interval=adjust_interval if adjust_interval is not None else 7
        self.pe_range=pe_range if pe_range is not None else [0,30]
    ...
    def stock_pool(self):
        ...
        last_phase_codes=[]
        #在调整日调整股票池
        for _index in range(0,len(all_dates),self.adjust_interval):
            #保存调整日
            ...
            daily_cursor=self.daily.find(
                {"date":adjust_date,"peTTM":{"$lt":self.pe_range[1],"$gt":self.pe_range[0]},"tradestatus":"1"},sort=[("peTTM",sort)],
            ...
            if len(last_phase_codes)>0:
                suspension_cursor=self.daily.find(
    ...
    def statistic_stock_pool(self):
        '''统计股票池的收益'''

        #找到指定时间范围内的股票池数据，这里的时间范围可以改变
        adjust_dates,codes_dict=self.stock_pool()
        ...
        hs300_begin_value=self.daily.find_one({"code":"sh.000300","index":True,"date":adjust_dates[0]})["close"]
        ...
        for _index in range(1,len(adjust_dates)-1):
            ...
            #构建股票代码和后复权买入价格的股票
            buy_daily_cursor=self.daily_hfq.find(
            ...
            当前的调整日，也就是上期的结束日
            '''
            sell_daily_cursor=self.daily_hfq.find(
            ...
            if count>0:
                ...
                #当前沪深300的值
                hs300_close=self.daily.find_one(
...
try:   
    get_ipython().system('jupyter nbconvert --to stock_pool_strategy.ipynb')
